...
import {
  WRAP, SECTION, SECTION_DARK, BTN_PRIMARY,
  SEC_EYEBROW, H_SEC, LEAD, U, DOT, WM, WM_LEFT, CARD, METRIC_STRIP, metricPill, METRIC_DOT,
} from '../cx'
import {
        ...
        <div className="mt-[clamp(32px,3.6vw,52px)] grid grid-cols-4 gap-[clamp(10px,1vw,16px)] max-[900px]:grid-cols-2 max-[520px]:grid-cols-1">
          {SERVICES_FULL.map((s, i) => (
            <Reveal
              ...
              className="group relative aspect-[4/5] overflow-hidden rounded border border-line dark:border-glass-brd max-[520px]:aspect-[16/10]"
            >
              <img
                src={s.img}
                alt={s.short}
                loading="lazy"
                className="absolute inset-0 h-full w-full object-cover transition-transform duration-[900ms] ease-smooth group-hover:scale-[1.07]"
              />
              {/* base legibility scrim */}
              <span aria-hidden="true" className="absolute inset-0 [background:linear-gradient(0deg,rgba(5,6,10,0.92)_4%,rgba(5,6,10,0.38)_44%,rgba(5,6,10,0.04)_80%)]" />
              {/* deepen on hover / touch so the expanded copy reads */}
              <span aria-hidden="true" className="absolute inset-0 bg-[#05060a]/35 opacity-0 transition-opacity duration-500 group-hover:opacity-100 max-[900px]:opacity-100" />
              {/* number badge */}
              <span className="absolute left-4 top-4 grid h-9 w-9 place-items-center rounded-[7px] bg-yellow font-cond text-[15px] font-semibold leading-none text-on-accent shadow-[0_6px_18px_-6px_rgba(0,0,0,0.6)]">
                {s.n}
              </span>
              {/* title + progressive detail */}
              <div className="absolute inset-x-0 bottom-0 p-[clamp(15px,1.3vw,22px)] text-white">
                <h3 className="font-display text-[clamp(17px,1.5vw,22px)] font-bold leading-[1.12] [text-shadow:0_2px_14px_rgba(0,0,0,0.55)]">
                  {s.short}
                </h3>
                <div className="grid grid-rows-[0fr] opacity-0 transition-all duration-[450ms] ease-smooth group-hover:grid-rows-[1fr] group-hover:opacity-100 max-[900px]:grid-rows-[1fr] max-[900px]:opacity-100">
                  <div className="min-h-0 overflow-hidden">
                    <p className="mt-2 text-[12.5px] leading-[1.5] text-white/85">{s.desc}</p>
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {s.tags.map((t) => (
                        <span key={t} className="rounded-full border border-white/25 px-2.5 py-[3px] font-mono text-[9.5px] uppercase tracking-[0.05em] text-white/85">
                          {t}
                        </span>
                      ))}
                    </div>
              ...
              {/* hover edge accent */}
              <span aria-hidden="true" className="pointer-events-none absolute inset-0 rounded border-2 border-yellow opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
            </Reveal>
          ))}
        </div>
      </div>
      ...
      <div className={WRAP}>
        <div className="max-w-[820px]">
          <SectionHead idx="02" kicker="Railway formation & rehabilitation">
            RDSO-grade <span className={U}>formation works</span>, end to end<em className={DOT}>.</em>
          </SectionHead>
          <p className={LEAD}>
        ...
        <Reveal className="mt-[clamp(28px,3vw,44px)] grid grid-cols-4 gap-3 max-[760px]:grid-cols-2">
          {RF_FIELD_STRIP.map((f) => (
            <div key={f.img} className="group relative aspect-[4/3] overflow-hidden rounded border border-line dark:border-glass-brd">
              <img className="h-full w-full object-cover transition-transform duration-[800ms] ease-smooth group-hover:scale-[1.06]" src={f.img} alt={f.alt} loading="lazy" />
              <span className="absolute inset-0 opacity-0 transition-opacity duration-300 [background:linear-gradient(0deg,rgba(5,6,10,0.45),transparent_60%)] group-hover:opacity-100" aria-hidden="true" />
            </div>
          ))}
        </Reveal>
    ...
    <section className={SECTION} id="critical">
      ...
      <div className={WRAP}>
        ...
        <Reveal className="mt-[clamp(28px,3vw,44px)] grid grid-cols-[0.62fr_1.4fr_1fr] gap-3 max-[900px]:grid-cols-2 max-[560px]:grid-cols-1">
          <div className="flex flex-col justify-between gap-6 rounded border border-line bg-paper p-[clamp(22px,2vw,30px)] dark:border-glass-brd dark:bg-glass dark:[backdrop-filter:blur(12px)] max-[900px]:col-span-2 max-[900px]:flex-row max-[560px]:col-span-1 max-[560px]:flex-col">
            <div>
          ...
          {CRITICAL_BANNER.map((b) => (
            <div key={b.img} className="relative h-[clamp(220px,30vw,320px)] overflow-hidden rounded border border-line dark:border-glass-brd max-[560px]:h-[220px]">
              <img className="h-full w-full object-cover" src={b.img} alt={b.alt} loading="lazy" />
            </div>
          ))}
        </Reveal>
          ...
          {CRITICAL_WORKS.map((c, i) => (
            <Reveal as="div" key={c.n} delay={i * 0.06} className="h-full">
              <SpotlightCard
                as="article"
                tilt={0}
                className={`${CARD} h-full p-[clamp(26px,2.4vw,38px)] before:absolute before:inset-x-0 before:top-0 before:z-[2] before:h-[3px] before:origin-left before:scale-x-0 before:bg-yellow before:transition-transform before:duration-[400ms] before:ease-smooth before:content-[''] hover:before:scale-x-100`}
              >
                <div className="relative z-[1] flex h-full flex-col">
                  ...
                  <span className="mt-5 inline-flex w-max items-center gap-2 rounded-full bg-yellow-soft px-3.5 py-1.5 font-mono text-[10.5px] uppercase tracking-[0.06em] text-ink dark:bg-yellow dark:text-on-accent">
                    {c.tag}
                  </span>
                </div>
              </SpotlightCard>
...
function ProjectDelivery() {
  return (
    <section className="relative py-[clamp(40px,6vw,96px)]" id="process">
      <div className={WRAP}>
        <div className={`${SECTION_DARK} overflow-hidden rounded-[clamp(16px,2vw,28px)] p-[clamp(36px,4.5vw,72px)_clamp(24px,4vw,64px)] shadow-[0_60px_120px_-50px_rgba(14,18,23,0.65)] dark:border dark:border-glass-brd`}>
      ...
      <div className={WRAP}>
        <div className="max-w-[760px]">
          <SectionHead idx="05" kicker="Safety & quality assurance">
            Safety. Quality. <span className={U}>Compliance</span><em className={DOT}>.</em>
          </SectionHead>
          <p className={LEAD}>
        ...
        <div className="mt-[clamp(28px,3vw,44px)] grid grid-cols-2 gap-5 max-[760px]:grid-cols-1">
          {SAFETY_STANDARDS.map((s, i) => (
            <Reveal as="div" key={s.n} delay={i * 0.06} className="h-full">
              <SpotlightCard as="article" tilt={0} className={`${CARD} h-full p-[clamp(26px,2.4vw,38px)]`}>
                <div className="relative z-[1]">
                  <span className="font-cond text-[15px] font-semibold tracking-[0.06em] text-yellow-deep">/{s.n}</span>
                  <h4 className="mt-3 font-display text-[clamp(18px,1.7vw,23px)] font-bold dark:text-text">{s.title}</h4>
                  <p className="mt-2.5 text-[14.5px] leading-[1.6] text-steel">{s.desc}</p>
                </div>
              </SpotlightCard>
            </Reveal>
          ))}
        ...
        <Reveal delay={0.1}>
          <div className={METRIC_STRIP}>
            {COMPLIANCE.map((c) => (
              <span className={metricPill('border-line-2')} key={c}><i className={METRIC_DOT} />{c}</span>
            ))}
          </div>
        </Reveal>
      </div>
...
function ServicesCTA() {
  return (
    <section className="relative pb-[clamp(48px,7vw,110px)] pt-[clamp(8px,1vw,16px)]">
      <div className={WRAP}>
        <div className={`${SECTION_DARK} flex flex-wrap items-center justify-between gap-8 rounded-[clamp(16px,2vw,28px)] p-[clamp(34px,4vw,64px)] dark:border dark:border-glass-brd`}>
          <div>
            ...
            <h2 className={`${H_SEC} max-w-[18ch] text-white`}>Ready to start a <span className={U}>build</span><em className={DOT}>?</em></h2>
          </div>
