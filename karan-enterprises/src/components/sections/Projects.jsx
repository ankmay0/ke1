...
import { gsap, MOTION_OFF } from '../../lib/motion'
...
function ProjectCard({ p, i }) {
  ...
  return (
    <article className="group relative aspect-[16/10] w-[clamp(300px,32vw,432px)] shrink-0 grow-0 overflow-hidden rounded-md bg-dark ring-1 ring-white/10 [scroll-snap-align:start] transition-[transform,box-shadow,ring-color] duration-[400ms] ease-smooth hover:z-20 hover:scale-[1.09] hover:shadow-[0_44px_80px_-26px_rgba(0,0,0,0.85)] hover:ring-white/25">
      {/* thumbnail */}
      <img
        src={p.img}
        alt={p.title}
...
export function Projects() {
  ...
  return (
    <section
      className={`relative isolate overflow-hidden bg-paper dark:bg-transparent ${pinned ? 'flex h-screen flex-col' : ''}`}
      id="projects"
      ref={pin}
    >
      <span className={WM} aria-hidden="true">05</span>
      <div className={`${WRAP} grid grid-cols-[1fr_auto] items-end gap-x-10 gap-y-[18px] pb-[clamp(30px,3.4vw,48px)] pt-[var(--sec)] max-[760px]:grid-cols-1 ${pinned ? 'flex-none' : ''}`}>
        ...
        <span className="col-span-full inline-flex items-center gap-2.5 font-display text-[12px] font-bold uppercase tracking-[0.16em] text-steel" aria-hidden="true">
          Scroll{' '}
          <span className="relative h-4 w-0.5 bg-current animate-scrollNudge motion-reduce:animate-none after:absolute after:-bottom-px after:left-1/2 after:h-[7px] after:w-[7px] after:-translate-x-1/2 after:rotate-45 after:border-b-2 after:border-r-2 after:border-current after:content-['']" />
        </span>
      </div>

      <div className={`overflow-x-auto overflow-y-hidden [scroll-snap-type:x_proximity] [-webkit-overflow-scrolling:touch] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${pinned ? 'flex flex-1 items-center overflow-hidden pb-0' : 'py-[clamp(20px,3vw,40px)]'}`}>
        <div className="flex w-max items-center gap-[18px] px-[var(--pad)]" ref={track}>
          {PROJECTS.map((p, i) => (
            <ProjectCard p={p} i={i} key={p.title} />
          ))}

          {/* Netflix-style end tile — the row's call to action */}
          <a
            href="#quote"
            className="group relative flex aspect-[16/10] w-[clamp(300px,32vw,432px)] shrink-0 grow-0 flex-col justify-between overflow-hidden rounded-md border border-glass-brd bg-dark p-6 text-white ring-1 ring-white/10 [scroll-snap-align:start] transition-[transform,box-shadow,ring-color] duration-[400ms] ease-smooth hover:z-20 hover:scale-[1.06] hover:shadow-[0_44px_80px_-26px_rgba(0,0,0,0.85)] hover:ring-yellow/40"
          >
